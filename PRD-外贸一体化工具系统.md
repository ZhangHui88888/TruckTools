...
**验收标准**:
- 以表格形式展示所有客户信息
- 包含字段：优先级、客户姓名、会面时间、会面地点、国家、手机号、邮箱、所属公司、公司官网、备注信息、微信二维码、WhatsApp二维码、跟进状态
- 默认按优先级降序排列（高优先级在前）
- 支持按任意字段排序和筛选
- 支持按跟进状态筛选（等待客户回复/等待我们回复/已完成）
- 支持添加自定义字段（用于后续邮件个性化）
- 支持编辑、删除客户信息
- 支持导出为Excel格式
- 点击客户可进入详情页查看完整信息
...
**Story 2.1: 客户详情与事件跟进** ✅ 已实现
> 作为一名销售人员，我希望能够查看客户的完整信息，并记录每次与客户的沟通事件，这样我就可以更好地跟进客户，不遗漏任何重要信息。

**验收标准**:
- 客户详情页展示完整客户信息
- 展示客户名片图片（正面/背面），支持点击放大
- 展示微信/WhatsApp二维码
- 显示跟进状态（等待客户回复/等待我们回复/已完成）
- 支持添加跟进事件（事件时间、地点、内容、状态）
- 以时间线形式展示所有事件记录
- 支持编辑和删除事件
- 显示邮件发送统计（已发送数量、最后发送时间）

---

**Story 2.2: 客户名片图片管理** ✅ 已实现
> 作为一名销售人员，我希望能够为客户上传名片图片（正面和背面），这样我就可以随时查看客户的原始名片信息，便于核对和记忆。
...
**Story 2.3: 工作台事件跟进管理**
> 作为一名销售人员，我希望在工作台中按时间顺序查看所有客户的事件，并能清晰区分需要我处理的事件和等待客户反馈的事件，这样我就可以高效地管理客户跟进工作，不遗漏任何重要事项。

**验收标准**:
- 工作台按时间顺序展示所有客户事件
- 需要我方处理的事件（状态：等待我方处理）排在前面，优先显示
- 等待客户反馈的事件排在后面
- 处理事件时，可新增事件内容记录处理结果
- 完成处理后，自动新增一条事件，状态设为"等待客户反馈"
- 客户超过3天未反馈时，系统自动新增事件"客户长时间未反馈，需要二次跟进"，状态改为"等待我方处理"
- 支持点击"不再跟进"按钮，标记该客户不再提醒二次跟进
- 标记"不再跟进"后，该客户不再触发超时提醒，直到产生新的事件才恢复提醒机制
- 事件列表支持按客户、状态、时间范围筛选
- 点击事件可跳转到对应客户详情页
...
**场景9: 工作台日常事件跟进**
1. 销售人员每天早上登录系统，进入工作台页面
2. 工作台显示所有待处理事件，按优先级排序：
   - 最上方显示"等待我方处理"的事件（红色标记）
   - 下方显示"等待客户反馈"的事件（灰色标记）
3. 销售人员看到一条"客户长时间未反馈，需要二次跟进"的提醒
4. 点击该事件，跳转到客户详情页
5. 查看客户历史沟通记录，决定发送跟进邮件
6. 新增事件记录："已发送二次跟进邮件，询问报价反馈"
7. 点击"完成处理"，系统自动新增事件，状态变为"等待客户反馈"
8. 返回工作台，该客户事件已移至"等待客户反馈"区域
9. 对于一个长期无响应的客户，销售人员点击"不再跟进"
10. 该客户不再出现超时提醒，直到客户主动联系产生新事件
...
**功能点2: 客户信息表格**
- 表格字段：
  - 系统字段：优先级、客户姓名、会面时间、会面地点、国家、手机号、邮箱、所属公司、公司官网、备注信息、跟进状态
  - 图片字段：微信二维码、WhatsApp二维码、名片正面、名片背面
  - 自定义字段：用户可添加任意文本字段
- 按优先级降序排列（默认）
- 支持按任意字段排序
- 支持多条件筛选（优先级、国家、公司、时间范围、跟进状态等）
- 支持关键词搜索（姓名、公司、邮箱）
- 支持单条/批量编辑、删除
- 支持导出为Excel格式

**功能点2.1: 客户详情页** ✅ 已实现
- 完整客户信息展示（基本信息、联系方式、会面信息等）
- 名片图片展示（正面/背面，支持点击放大预览）
- 微信/WhatsApp二维码展示
- 跟进状态管理（等待客户回复/等待我们回复/已完成）
- 事件记录时间线（支持添加、编辑、删除事件）
- 邮件发送统计（已发送数量、最后发送时间）
...
| 跟进状态/Follow Up Status | 跟进状态 | - |

**客户数据库字段说明** ✅ 已实现:

| 字段名 | 类型 | 说明 |
|--------|------|------|
| business_card_front | VARCHAR(500) | 名片正面图片路径 |
| business_card_back | VARCHAR(500) | 名片背面图片路径 |
| follow_up_status | VARCHAR(50) | 跟进状态：pending_customer/pending_us/completed |
...
| event_status | VARCHAR(50) | 事件状态：pending_customer/pending_us |
...
工作台是销售人员的日常工作入口，集中展示所有需要处理的客户事件，帮助销售人员高效管理客户跟进工作。

**功能点16: 事件跟进列表**

以时间线形式展示所有客户事件，按优先级和时间排序，方便销售人员快速处理待办事项。

**页面布局**:
- 顶部：筛选条件（客户名称、事件状态、时间范围）
- 主体：事件列表，分为两个区域
  - 上方：需要我方处理的事件（红色/橙色标记，优先显示）
  - 下方：等待客户反馈的事件（灰色标记）
- 右侧：快捷操作面板
...
| 字段名 | 说明 |
|--------|------|
| 客户名称 | 关联的客户姓名，点击可跳转详情页 |
| 所属公司 | 客户所属公司 |
| 事件时间 | 事件发生/创建时间 |
| 事件内容 | 事件描述（最多显示100字，超出截断） |
| 事件状态 | 等待我方处理 / 等待客户反馈 |
| 等待天数 | 距离上次更新的天数（超过3天标红） |
| 操作 | 处理、查看详情、不再跟进 |

**排序规则**:
1. 首先按事件状态排序：等待我方处理 > 等待客户反馈
2. 同状态内按时间倒序排列（最新的在前）
3. 超时未反馈的事件置顶显示
...
4. 系统自动执行：
   - 保存当前处理记录
   - 新增一条事件，内容为处理记录，状态设为"等待客户反馈"
   - 更新客户跟进状态为"等待客户回复"

**状态流转规则**:
```
等待我方处理 --[完成处理]--> 等待客户反馈
等待客户反馈 --[客户回复/新增事件]--> 等待我方处理
等待客户反馈 --[超过3天未反馈]--> 自动新增"二次跟进"事件 --> 等待我方处理
```

**处理对话框字段**:
| 字段名 | 类型 | 是否必填 | 说明 |
|--------|------|---------|------|
| 处理内容 | 文本域 | 是 | 记录本次处理的具体内容 |
| 处理时间 | 日期时间 | 是 | 默认当前时间，可修改 |
| 附件 | 文件上传 | 否 | 支持上传相关文档 |
...
系统自动检测等待客户反馈超过3天的事件，自动创建二次跟进提醒。

**自动提醒规则**:
- 触发条件：事件状态为"等待客户反馈"且超过3天（72小时）未更新
- 触发动作：
  1. 自动新增一条事件，内容为"客户长时间未反馈，需要二次跟进"
  2. 新事件状态设为"等待我方处理"
  3. 更新客户跟进状态为"等待我们回复"
- 执行频率：每天凌晨2:00执行定时任务检查

**提醒内容模板**:
```
【系统提醒】客户 {客户姓名} 已超过 {N} 天未反馈
上次沟通时间：{上次事件时间}
上次沟通内容：{上次事件内容摘要}
建议：请及时跟进，了解客户最新情况
```
...
**技术实现**:
- 后端定时任务（Cron Job）每日执行
- 检查 customer_events 表中状态为 pending_customer 且 updated_at < NOW() - 3天 的记录
- 排除已标记"不再跟进"的客户
...
支持对长期无响应的客户标记"不再跟进"，停止自动提醒。

**功能说明**:
- 点击"不再跟进"按钮后，弹出确认对话框
- 确认后，该客户不再触发超时自动提醒
- 在客户列表和详情页显示"已停止跟进"标记
- 当该客户产生新的事件时（如客户主动联系），自动恢复跟进提醒机制

**数据库字段**:
| 字段名 | 类型 | 说明 |
|--------|------|------|
| stop_follow_up | BOOLEAN | 是否停止跟进，默认 false |
| stop_follow_up_time | DATETIME | 停止跟进时间 |
| stop_follow_up_reason | VARCHAR(500) | 停止跟进原因（可选） |

**恢复跟进条件**:
- 用户手动新增事件
- 客户主动联系（如回复邮件，需要后续集成邮件系统）
- 用户手动点击"恢复跟进"
...
在工作台顶部展示关键统计数据，帮助销售人员快速了解工作状态。

**统计卡片**:
| 指标名称 | 说明 |
|---------|------|
| 待处理事件 | 状态为"等待我方处理"的事件数量 |
| 等待反馈 | 状态为"等待客户反馈"的事件数量 |
| 今日处理 | 今天完成处理的事件数量 |
| 超时未跟进 | 超过3天未反馈的客户数量 |
| 本周新增客户 | 本周新增的客户数量 |
| 活跃客户 | 近7天有事件更新的客户数量 |
...
| 字段名 | 类型 | 说明 |
|--------|------|------|
| is_system_generated | BOOLEAN | 是否系统自动生成（如超时提醒） |
| parent_event_id | BIGINT | 关联的父事件ID（用于追溯） |
...
| stop_follow_up | BOOLEAN | 是否停止跟进，默认 false |
| stop_follow_up_time | DATETIME | 停止跟进时间 |
| stop_follow_up_reason | VARCHAR(500) | 停止跟进原因 |
| last_event_time | DATETIME | 最后事件时间（冗余字段，便于查询） |
| pending_event_count | INT | 待处理事件数量（冗余字段，便于统计） |
...
| v1.2 | 2025-12-06 | [产品经理] | 客户管理模块增强：新增客户详情页、名片上传功能、客户事件管理、跟进状态管理 |
| v1.3 | 2025-12-06 | [产品经理] | 新增工作台模块：事件跟进列表、事件处理与状态流转、超时自动提醒机制（3天未反馈自动提醒二次跟进）、不再跟进标记、工作台数据统计 |
